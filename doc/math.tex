...
A \textit{basis} is a set of functions, call them $\{\xi_k\}$, that can be summed together in various quantities to produce other functions. Often these are chosen to be orthogonal, meaning that if we take the ``inner product" of one funtion from the set with itself, we get back a constant (often normalized to be 1), and if we take the inner product of a function with a different member of the set, we get back 0. In this sense the members of an orthogonal basis set are independent of one another, just like perpendicular directions on a graph.
