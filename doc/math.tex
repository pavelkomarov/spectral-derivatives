...
where $\vec{Y}^{(\nu)}_{\text{II}}$ are found completely analogously to $\vec{Y}^{(\nu)}$, via the first half of \autoref{Y_nu}, except there is no Nyquist ($k = \frac{M}{2} = N+1$) term, so we only have to consider $k < \frac{M}{2}$.
...
For both orders above, if we instead use type II reconstruction, then $Y_k$ are really $Y_{k,\text{II}}$, and the only difference is that the $k = N$ terms are multiplied by 2 like every other, instead of by 1, and can therefore be moved \textit{inside} the sums.
...
where the alternating plus and minus in the $k$ and $N$ terms comes from the fact the $2^{nd}$ derivative contains $-$cosines, the $4^{th}$ +cosines, the $6^{th}$ $-$cosines again, and so on. Once again, if we use type II reconstruction, the $k=N$ terms disappear inside the sums, which then go up to $N$ instead of $N-1$.
