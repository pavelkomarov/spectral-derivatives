...
To see where it comes from, observe it's possible to sample the periodic extension of $y$ in other ways. If we choose $\theta_{n,\text{II}} = \frac{2\pi}{M}(n + \frac{1}{2}),\ n \in \{0, ... M-1\}$, then we get:
...
This is the DCT-II$^{-1}$ if we take its coefficients to be the phase ramp multiplied with the DFT coefficients. Operating on samples $y_{n,\text{II}}$ taken from $\theta_{n,\text{II}}$ \textit{offsets} the phase difference and makes this equivalent to a DFT, where we once again get the benefit of only needing non-redundant information. Notice the Nyquist term, which would be at index $\frac{M}{2} = N+1$, has disappeared with this sampling. Also notice this sampling \textit{doesn't include the domain endpoints}.

If we take away the $\frac{1}{M}$ and a flip $Y$ and $y$, this is the same as the forward DCT-III. And just as before, since we can use the inverse transform to reconstruct the signal from coefficients, we can use the forward DCT-II transform to obtain the basis coefficients.
