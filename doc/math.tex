...
Notice all the even-power terms appear with alternating sign as in the cosine expansion, and the odd-power terms appear with alternating sign as in the sine expansion, but with an extra $j$ multiplied in.
...
The set of numbers $\{c_k\}$ is now \textit{an alternative representation} of the original function. In some sense it's equally descriptive, so long as we know which basis we're using to reconstruct. The function has been \textit{transformed}.
...
\begin{figure}[h!]
	...
	\caption*{An example of the Gibbs phenomenon, from \cite{kutz}. Using more modes allows the solution to fit better in the middle of the domain but makes peaks at the edges more extreme.}
\end{figure}
...
\begin{tikzpicture}[overlay, remember picture, scale=0.8]
	\draw[->, thick] (-8.7, 5.5) -- (-6.5, 5.5) node[midway, above] {\parbox{12mm}{flip and warp}};
...
where the alternating plus and minus in the $k$ and $N$ terms comes from the fact the $2^{nd}$ derivative contains $-$cosines, the $4^\text{th}$ +cosines, the $6^\text{th}$ $-$cosines again, and so on.
