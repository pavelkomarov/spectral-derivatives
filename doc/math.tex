...
The DCT-I is actually not the default in \texttt{scipy}\cite{dct}, because the DCT-II better matches the Karhunen–Loève Transform\cite{klt}, which is provably the optimal basis to represent signals generated by a random process that obeys the Markov property (where values are related only to \textit{immediate} previous values, through an autocorrelation coefficient). This class of signals is not exactly the same as the class of smooth functions, but it's similar in spirit, especially in the presence of noise. Thus we can often get better ``energy compaction" with this basis and represent signals with fewer coefficients\cite{energycompaction}. Indeed, common compression standards like JPEG choose to use the DCT-II.
