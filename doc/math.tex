...
The most famous basis is the \textit{Fourier} basis\endnote{There's a great passage in \href{https://en.wikipedia.org/wiki/Richard_Hamming}{Richard Hamming}'s book \textit{The Art of Doing Science and Engineering}\cite{hamming} where he wonders why we use the Fourier basis so much:
...
where $M$ is the number of basis functions I'm using in my reconstruction and $k$ iterates through them.\newline
...
$$ Y_{M/2}(ue^{j\frac{M}{2}\theta} + (1 - u)e^{-j\frac{M}{2}\theta}) $$\vspace{-5mm}
...
Because the reconstruction of $y(\theta)$ (\autoref{equivalent}) only contains cosines, doing a full FFT/DFT, which tries to fit sines as well as cosines, would be doing double work. Instead we can use the DCT. There are actually \href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.fft.dct.html}{several possible definitions of the DCT}\cite{dct}; we'll start with the DCT-I, because its coefficients match those of the DFT without a \hyperref[phaseramp]{``phase ramp"} or any \hyperref[halfindex]{funky half-index sampling}, which makes it simpler to understand.

Say we ``periodically extend" $y$, i.e.~stack $y(\theta),\ \theta \in [0,\pi]$ next to a horizontal flip of itself on $(\pi, 2\pi)$, and then sample at the canonical DFT points, $\theta_n = \frac{2\pi}{M}n,\ n \in \{0, ... M-1\}$ (\autoref{dft}). We get:\vspace{-2mm}

$$\vec{y}_{\text{ext}} = \underbrace{[\underbrace{y_0, y_1, ... y_{N-1}, y_N}_{\parbox{20mm}{\footnotesize \centering original vector, length N+1}}, \underbrace{y_{N-1}, ... y_1}_{\parbox{16mm}{\footnotesize \centering redundant information}}]}_{\text{length M = 2N, necessarily even!}}, \text{ that is: } y_n = y_{M-n},\ 0 \leq n \leq N$$
...
\subsubsection{DCT Type II}

The DCT-I is actually not the default in \texttt{scipy}\cite{dct}, because the DCT-II better matches the Karhunen–Loève Transform, which is \href{https://www.farbrausch.de/%7Efg/articles/dct_klt.pdf}{provably the optimal basis}\cite{klt} for representation, of signals generated by a random process that obeys the Markov property (where values are related only to \textit{immediate} previous values, through an autocorrelation coefficient). This class of signals is not exactly the same as the class of smooth functions, but it's similar in spirit, especially in the presence of noise. Thus we can often get better ``energy compaction" with this basis and \href{https://dsp.stackexchange.com/a/96197/40873}{represent signals in fewer coefficients}\cite{energycompaction}. Indeed, common compression standards like JPEG choose to use the DCT-II.

To see where it comes from, observe it's possible to sample the periodic extension of $y$ in other ways. If we choose $\theta_{n,\text{II}} = \frac{2\pi}{M}(n + \frac{1}{2}),\ n \in \{0, ... M-1\}$, then we get:\vspace{-2mm}

$$\vec{y}_{\text{ext,II}} = \underbrace{[\underbrace{y_{0,\text{II}}, ... y_{N,\text{II}}}_{\parbox{20mm}{\footnotesize \centering original vector, length N+1}}, \underbrace{y_{N,\text{II}}, ... y_{0,\text{II}}}_{\parbox{16mm}{\footnotesize \centering redundant information}}]}_{\makebox[0pt]{\footnotesize length M = 2(N+1), necessarily even!}}, \text{ that is: } y_{n,\text{II}} = y_{M-n-1,\text{II}},\ 0 \leq n \leq N$$

If we once again use $M-k$ instead of $k$ in the DFT equation (\autoref{dft}), we can find the relationship between $Y_{M-k}$ and $Y_k$:
...
&= \sum_{n=0}^{M-1} \underbrace{y_{M-n-1,\text{II}}}_{= y_{n,\text{II}}} \underbrace{e^{j 2\pi k}}_{1} e^{-j\frac{2\pi}{M}(n+1)k} = e^{-j \frac{2\pi}{M} k} \sum_{n=0}^{M-1} y_{n,\text{II}} e^{-j \frac{2\pi}{M}nk} = e^{-j \frac{2\pi}{M} k} Y_k \quad \square
\end{align*}
\begin{tikzpicture}[overlay, remember picture]
	\node at (11.45,3.25) {\footnotesize let $n_{new} = M - n_{old} - 1$};
	\draw[->] (11.15,3.05) -- (11,2.5);
\end{tikzpicture}

Let's now plug these in to the interpolant (\autoref{interpolant}):

\begin{equation}\label{phaseramp}
\begin{aligned}
y(\theta) &= \frac{1}{M} \Big(\underbrace{Y_0}_{\makebox[0pt]{\footnotesize $=\!\underbrace{e^{-j \frac{2\pi}{M} 0}}_{1} Y_0$}\hspace{4mm}} + \sum_{0 < k < \frac{M}{2}} \big( Y_k e^{j k \theta} + \underbrace{Y_{M-k}}_{\makebox[0pt]{\footnotesize $=\!e^{-j \frac{2\pi}{M} k} Y_k$}\hspace{6mm}} e^{-j k \theta}\big) + \underbrace{Y_{M/2}}_{\hspace{9mm}\makebox[0pt]{\footnotesize $=\!\underbrace{e^{-j \frac{2\pi}{M} \frac{M}{2}}}_{-1} Y_{M/2} \rightarrow Y_{M/2} = 0$}}\cos(\frac{M}{2}\theta) \Big) \\
...
The DFT is blind to where samples came from and assumes they are at $\theta_n = \frac{2\pi}{M}n = \frac{\pi}{N+1}n$, which results in:\vspace{-2mm}

\[\label{halfindex}
y_{n,\text{II}} = \frac{1}{M} \Big(\underset{\makebox[0pt]{$\substack{\uparrow \\ Y_{0,\text{II}}}$}}{Y_0} + 2 \sum_{k = 1}^N \underbrace{Y_k \underbrace{e^{-j \frac{2\pi}{M}k\frac{1}{2}}}_{\text{phase ramp}}}_{Y_{k,\text{II}}} \cos\!\big(\underbrace{\frac{\pi k}{N+1} (n + \frac{1}{2})}_{=\theta_{n,\text{II}}}\big) \Big) \tag{DCT-II$^{-1}$}
\]

This is the DCT-II$^{-1}$ if we take its coefficients to be the phase ramp multiplied with the DFT coefficients. Operating on samples $y_{n,\text{II}}$, taken from $\theta_{n,\text{II}}$ (``half-indices"), \textit{offsets} the phase difference and makes this equivalent to a DFT, where we once again get the benefit of only needing non-redundant information. Notice the Nyquist term, which would be at index $\frac{M}{2} = N+1$, has disappeared with this sampling, its positive and negative frequency components perfectly cancelling due to interaction with the phase ramp.

Just as before, since we can use the \textit{inverse} transform to reconstruct the signal from basis coefficients, we can use the \textit{forward} DCT-II transform to obtain those coefficients. If we take away the $\frac{1}{M}$ and a flip $Y$ and $y$, the inverse transform is the same as the forward DCT-III.
...
\subsubsection{Inverse Transforms with DCT-II}

We can follow a similar (and now that the even-odd intuition has been covered, maybe a more direct) line of reasoning to find inverse transforms for derivatives when using the DCT-II. So far we know:
...
If we substitute a continuous $\theta$ for $\theta_{n,\text{II}}$, then we can obtain the continuous reconstruction:

$$y(\theta) = \frac{1}{M} \Big( Y_{0,\text{II}} + 2 \sum_{k=1}^N Y_{k,\text{II}} \cos\!\big(k \theta \big) \Big)$$

We can then simply take its derivative w.r.t. $\theta$:

\begin{align*}
y'(\theta) &= \frac{1}{M} \Big( -2 \sum_{k=1}^N k Y_{k,\text{II}} \sin\!\big(k \theta \big) \Big) = \frac{1}{M} \Big( 2 \sum_{k=1}^N \underbrace{jk Y_{k,\text{II}}}_{Y'_{k,\text{II}}} \cdot j \sin\!\big(k \theta \big) \Big)\\
y''(\theta) &= \frac{1}{M} \Big( -2 \sum_{k=1}^N k^2 Y_{k,\text{II}} \cos\!\big( k \theta \big) \Big) = \frac{1}{M} \Big( 2 \sum_{k=1}^N \underbrace{(jk)^2 Y_{k,\text{II}}}_{Y''_{k,\text{II}}} \cos\!\big( k \theta \big) \Big)\\
&...
\end{align*}

We can continue the pattern, re-discretize $\theta_{n,\text{II}}$, and explicitly list out some zero terms to obtain something that looks like discrete inverse transforms for the even and odd cases:

\begin{align}
y_{n,\text{II}}^{(\text{odd } \nu)} &= \frac{1}{M} \Big( \underbrace{\underbrace{(-1)^n}_{\hspace{5mm} \makebox[0pt]{\footnotesize $\sin\!\big(\frac{\pi (N + 1)}{N+1} (n + \frac{1}{2})\big)$}} \cdot \overset{\makebox[3mm]{$\substack{Y^{(\nu)}_{N+1,\text{II}}\\ \downarrow}$}}{0} + 2 \sum_{k=1}^N Y^{(\nu)}_{k,\text{II}} \cdot j \sin\!\big(\frac{\pi k}{N+1}(n + \frac{1}{2})\big)}_{\text{\normalsize = a DST-III of $\vec{Y}^{(\nu)}_{\text{II}}$!}} \Big) \label{y_prime_odd_ii}\\
y_{n,\text{II}}^{(\text{even } \nu)} &= \frac{1}{M} \Big( \underbrace{\overset{\makebox[0pt]{$\substack{Y^{(\nu)}_{0,\text{II}}\\ \downarrow}$}}{0} + 2 \sum_{k=1}^N Y^{(\nu)}_{k,\text{II}} \cos\Big(\frac{\pi k}{N + 1}(n + \frac{1}{2})\Big)}_{\text{\normalsize = a DCT-III of $\vec{Y}^{(\nu)}_{\text{II}}$!}} \Big) \label{y_prime_even_ii}
\end{align}

where $\vec{Y}^{(\nu)}_{\text{II}}$ are found completely analogously to $\vec{Y}^{(\nu)}$, via the first half of \autoref{Y_nu}, except there is no Nyquist term ($k = \frac{M}{2} = N+1$), so we only have to consider $k < \frac{M}{2}$.

Note again that the DCT-III and DST-III definitions given in \texttt{scipy}\cite{dct}\cite{dst} use slightly different indexing and symbols than in my definitions here.

Importantly, the DST-III takes $N$ elements, but due to a +1 offset of the summed-over indexer, this is effectively working on elements $k \in \{1, ... N+1\}$. This is because for this variant of the DST there is a zero crossing at $k = 0$, so that entry would have no informational content. Because we need the $k = N+1$ element to be 0, and multiplying coefficients by $(jk)^\nu$ will give us elements $k \in \{0, ... N\}$ with the $k=0$ element 0, we can obtain what we need by \texttt{roll()}ing the coefficients to the left. The DCT-III is much more straightforward, taking elements $k = \{0, ... N\}$, just like the DCT-I.
...
\begin{align*}
	...
	& \longrightarrow -\sin(\theta) = -\sqrt{1 - x^2} \longrightarrow \frac{d\theta}{dx} = \frac{1}{\frac{dx}{d\theta}} = \frac{-1}{\sqrt{1 - x^2}} \quad \Big(\text{agrees with } \frac{d}{dx} \cos^{-1}(x) = \frac{-1}{\sqrt{1 - x^2}} \Big)
\end{align*}

The $\frac{d}{d\theta} y(\theta)$ a.k.a $y'(\theta)$ term is actually pretty easy to handle, because we know its value (and for higher orders too) at discretized $\theta_n$ from earlier! (Equations \ref{y_prime_odd} \& \ref{y_prime_even} or \ref{y_prime_odd_ii} \& \ref{y_prime_even_ii}) If we use the sampled $x_n = \cos(\theta_n)$ from step 1 of \hyperref[algo]{the algorithm} (or $x_{n,\text{II}} = \cos(\theta_{n,\text{II}})$, \textit{so long as the samplings agree!}), then our $\{x_n\}$ and $\{\theta_n\}$ align, and we can find samples of the derivative w.r.t. $x$ by plugging $\{x_n\}$ in to the new factor(s) and multiplying appropriately (pointwise):
...
Let's see it in fine detail for the $1^{st}$ derivative. The below uses the DCT-I reconstruction (\autoref{reconstruction}) for $y(\theta)$. The process is exactly the same if we use the DCT-II reconstruction instead (\autoref{phaseramp}), in some ways actually easier due to the absence of the Nyquist term and the inert quality of phase when taking derivatives of sinusoids.
...
For both orders above, if we instead use type II reconstruction, then $Y_k$ are really $Y_{k,\text{II}}$, and the only difference is the $k = N$ terms are multiplied by 2 like every other, instead of by 1, and can therefore be moved \textit{inside} the sums.
...
where the alternating plus and minus in the $k$ and $N$ terms comes from the fact the $2^{nd}$ derivative contains $-$cosines, the $4^{th}$ +cosines, the $6^{th}$ $-$cosines again, and so on. If we use type II reconstruction, the $k=N$ terms disappear inside the sums, which then go up to $N$ instead of $N-1$.
...
It is possible to sidestep all these ballooningly-complicated variable mappings and higher derivative limit-evaluations by building up to higher derivatives from lower ones in stages (being careful to only use even derivatives if we're using the DCT-I and our end goal is even, so we preserve the Nyquist term). This would entail more forward transforms but actually no more inverse ones, since we have to remember the results of all the inverse transforms up to the $\nu^{th}$ order anyway.
...
This is made more challenging by the fact our analytic reconstruction expression is based on the DCT-I$^{-1}$ or DCT-II$^{-1}$, which have terms outside the central sum, so as we substitute the DCT in to the DCT, we get ever more terms ($3^D$ of them for the DCT-I$^{-1}$ and $2^D$ of them for the DCT-II$^{-1}$), e.g.~in 2D the DCT-I$^{-1}$ is:
...
\begin{align}
x_n \in \cos\!\Big(\frac{\pi \{0, ... N\}}{N}\Big) &\leftrightarrow t_n \in \cos\!\Big(\frac{\pi \{0, ... N\}}{N}\Big) \cdot \frac{b - a}{2} + \frac{b + a}{2} \tag{type I}\\
x_{n,\text{II}} \in \cos\!\Big(\frac{\pi}{N+1}\big(\{0, ... N\}+\frac{1}{2}\big)\Big) &\leftrightarrow t_{n,\text{II}} \in \cos\!\Big(\frac{\pi}{N+1}\big(\{0, ... N\}+\frac{1}{2}\big)\Big) \cdot \frac{b - a}{2} + \frac{b + a}{2} \tag{type II}
\end{align}

Notice the order has flipped here, that counting \textit{up} in $n$ means we traverse $x$ from $+1 \rightarrow -1$. This is actually what we want; it corresponds to the \hyperref[cylinder]{horizontal flip} necessary to make cosine shadows equate with Chebyshev polynomails. Also notice the type II sampling \textit{doesn't naturally include the endpoints a and b}, which we may want, especially for consistency with the fact the domain is the \textit{closed} set $[a, b]$. We can correct for this by tacking on $\pm 1$ to the ends of $x_{n,\text{II}}$, which is the same as tacking on $a$ and $b$ at the ends of $t_{n,\text{II}}$.
...
To enable calculation of the \texttt{scale}, and to double check the user sampled their function at a correct \texttt{t\char`_n} (especially in the Chebyshev case, since cosine-spacing is easy to flub and especially confusing with the DCT-II), the functions take the sample locations \href{https://pavelkomarov.com/spectral-derivatives/specderiv.html}{as a parameter} and raise errors messages with correct examples if the sampling is invalid.
