...
\usepackage{amsmath, amssymb, amsfonts, bbm, hyperref, enumitem, enotez, tikz, pgfplots, cancel, graphicx,
caption, algorithm, algorithmic}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.58\textwidth, height=0.43\textwidth]{runge.png}
...
\end{align*}
\begin{tikzpicture}[overlay, remember picture]  % overlay ensures this drawing goes over the equation
...
This is exactly DCT-I$^{-1}$, which, except for the $\frac{1}{M}$ term and a flip of $Y$ and $y$, is the same as the forward DCT-I! But the DCT$^{-1}$ operates on the shorter set of $Y = [Y_0, ... Y_N]$, without redundant information. Thus
...
If we now gather the $y^{(\mu)}(\theta)$ terms and use the fact $\frac{d\theta}{dx} = \frac{-1}{\sqrt{1-x^2}}$, we can find its new miltiplying $f(x)$ is equal to:
...
\subsubsection{Handling Domain Endpoints}
...
In principle it is possible to build up the formulas via a similar process to the \hyperref[pyramid]{pyramid scheme} from earlier, because $f(x)$ factors multiplying respective orders of $y(\theta)$ come from at most two terms in the previous expression (before the latest application of L'Hôpital). But this time the $f(x)$ involve fractional powers of $\sqrt{1-x^2}$, which can't be as easily represented in \texttt{numpy}. So for now a neat scheme \href{https://github.com/pavelkomarov/spectral-derivatives/issues/1}{hasn't been invented}.

It is possible to sidestep the ballooningly-complicated variable mappings and limit evaluations of higher derivatives by building up to higher derivatives from lower ones in stages (being careful to only use even derivatives if our end goal is even, so we preserve the Nyquist term). This would involve more forward transforms but actually no more inverse ones, since we have to remember the results of all the inverse transforms up to the $\nu^{th}$ order anyway.

\section{Multidimensionality}

We are now fully equipped to find derivatives for 1-dimensional data. This is in fact all we need, because, due to linearity of the derivative operator, we can find the derivative along a particular dimension of a multidimensional space by using our 1D solution along each constituent vector running in that direction, and we can find derivatives along multiple dimensions by applying the above in series along each dimension:
...
\begin{tikzpicture}
	...
	\node[draw, rectangle, minimum width=3cm, minimum height=3cm, right=of Y] (jkY) {$(jk)^\nu \odot Y$};
...
That's really neat, but does it save us anything?
...
Now using this to evaluate the original:
...
But worst of all, at the edges of the domain we still need to use L'Hôpital's rule on an analytic expression 

If we start totaling up the amount of work this entails, we can see there's an extra $O(D \cdot N^D \log N)$ of work for that second term, and the situation only gets worse if we have higher derivatives that cause more splintering (\autoref{derivatives}), especially if we have them in more dimensions, because then we have to deal with \textit{the Cartesian product} of splintered terms! No good! 
