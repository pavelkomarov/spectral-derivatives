...
Because the reconstruction of $y(\theta)$ (\autoref{equivalent}) only contains cosines, doing a full FFT, which tries to fit sines as well as cosines, would be doing double work. Instead we can use the DCT. There are actually \href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.fft.dct.html}{several possible definitions of the DCT}\cite{dct}; we'll start with the DCT-I, because its coefficients match those of the DFT without a ``\hyperref[phaseramp]{phase ramp}", which makes it simpler to understand.

Say we ``periodically extend" $y$, i.e.~stack $y(\theta),\ \theta \in [0,\pi]$ next to a horizontal flip of itself on $(\pi, 2\pi)$, and then sample at $\theta_n = \frac{2\pi}{M}n,\ n \in \{0, ... M-1\}$. We get:\vspace{-2mm}
...
\centering
\begin{tikzpicture}
	\node (y_long) at (0, 2.75) {$\vec{y}_{\text{long}}$};
	\node (Y_long) at (0, 0) {$\vec{Y}_{\text{long}}$};
	\node (y_short) at (4, 2.75) {$\vec{y}_{\text{short}}$};
	\node (Y_short) at (4, 0) {$\vec{Y}_{\text{short}}$};
	...
	\draw[->, bend left=20] (y_long) to node[midway, above, yshift=-0.5mm] {truncate} (y_short);
	\draw[->, bend left=20] (y_short) to node[midway, below, yshift=1mm] {truncate$^{-1}$} (y_long);
	\draw[->, bend left=20] (Y_long) to node[midway, above, yshift=-0.5mm] {truncate} (Y_short);
	\draw[->, bend left=20] (Y_short) to node[midway, below, yshift=1mm] {truncate$^{-1}$} (Y_long);
\end{tikzpicture}
\end{minipage}
...
To see where it comes from, observe that it's possible to sample the periodic extension of $y$ in other ways. If we choose $\theta_n = \frac{2\pi}{M}(n + \frac{1}{2}),\ n \in \{0, ... M-1\}$, then we get:

$$\vec{y} = \underbrace{[\underbrace{y_0, y_1, ... y_{N-1}, y_N}_{\parbox{20mm}{\footnotesize \centering original vector, length N+1}}, \underbrace{y_N, y_{N-1}, ... y_1, y_0}_{\parbox{16mm}{\footnotesize \centering redundant information}}]}_{\text{length M = 2(N+1), necessarily even!}}, \text{ that is: } y_n = y_{M-n-1},\ 0 \leq n \leq N$$
...
\begin{align*}
Y_{M-k} &= \sum_{n=0}^{M-1} y_n e^{-j \frac{2\pi}{M}n(M-k)} = \sum_{n=0}^{M-1} y_n \underbrace{e^{-j 2\pi n}}_{1} e^{j \frac{2\pi}{M}nk} = \sum_{n = M-1}^{0} y_{M-n-1} e^{j \frac{2\pi}{M}(M - n - 1)k} \\
&= \sum_{n=0}^{M-1} \underbrace{y_{M-n-1}}_{= y_n} \underbrace{e^{j 2\pi k}}_{1} e^{-j\frac{2\pi}{M}(n+1)k} = e^{-j \frac{2\pi}{M} k} \sum_{n=0}^{M-1} y_n e^{-j \frac{2\pi}{M}nk} = e^{-j \frac{2\pi}{M} k} Y_k \quad \square
\end{align*}
\begin{tikzpicture}[overlay, remember picture]
	\node at (11.2,3.25) {\footnotesize let $n_{new} = M - n_{old} - 1$};
	\draw[->] (10.95,3.05) -- (10.85,2.5);
\end{tikzpicture}
...
At DFT samples $\theta_n = \frac{2\pi}{M}n = \frac{\pi}{N+1}n$, this becomes:\vspace{-2mm}

\[
y(\theta_n) = \frac{1}{M} \Big(Y_0 + 2 \sum_{k = 1}^N Y_k \underbrace{e^{-j \frac{2\pi}{M}k\frac{1}{2}}}_{\text{phase ramp}} \cos\!\big(\frac{\pi k}{N+1} (n + \frac{1}{2})\big) \Big) \tag{DCT-II$^{-1}$}
\]

This is the DCT-II$^{-1}$ if we take its coefficients to be the ``phase-ramp" multiplied with the DFT coefficients. If we take away the $\frac{1}{M}$ and a flip $Y$ and $y$, this is the same as the forward DCT-III. Once again we get to work without redundant information.
...
\end{align}
\begin{tikzpicture}[overlay, remember picture]
	\node at (7.2,7.3) {\parbox{13mm}{\footnotesize from oddness of $\tilde{\mathbbm{1}}^\nu$}};
	\node at (7.25,4.05) {\parbox{14mm}{\footnotesize from evenness of $\tilde{\mathbbm{1}}^\nu$}};
	\draw[->, blue] (7.7,7.6) -- (7.95,7.9);
	\draw[->, red] (7.8,4.3) -- (8.1,4.55);
\end{tikzpicture}
...
In other words, if we want to take a spectral derivative of a function that's periodic on $[a, b)$, then we need to sample it at \texttt{t\char`_n = np.arange(M)/M * (b - a) + a = np.linspace(a, b, M, endpoint=False)}.
