...
Where $[:\!N+1]$ truncates to only the first $N+1$ elements ($\{0, ... N\}$). Given the equality above, we can line up everything we now know in a diagram:\vspace{-3mm}
...
\centering
\begin{tikzpicture}
	\node (y_long) at (0, 3) {$\vec{y}_{\text{long}}$};
	\node (Y_long) at (0, 0) {$\vec{Y}_{\text{long}}$};
	\node (y_short) at (4, 3) {$\vec{y}_{\text{short}}$};
...
\end{align}
\begin{tikzpicture}[overlay, remember picture]
	\node at (7.2,7.15) {\parbox{13mm}{\footnotesize from oddness of $\tilde{\mathbbm{1}}^\nu$}};
	\node at (7.25,3.9) {\parbox{14mm}{\footnotesize from evenness of $\tilde{\mathbbm{1}}^\nu$}};
	\draw[->, blue] (7.7,7.45) -- (7.95,7.7);
	\draw[->, red] (7.8,4.15) -- (8.1,4.4);
\end{tikzpicture}
