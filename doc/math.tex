...
where $j$ to represents the imaginary unit, because I'm from Electrical Engineering, and because Python uses \texttt{j}.\newline
...
\begin{tikzpicture}[overlay, remember picture]
	\draw[->, thick] (-8.5, 5.5) -- (-7, 5.5) node[midway, above] {\parbox{12mm}{flip and warp}};
...
To see where it comes from, observe it's possible to sample the periodic extension of $y$ in other ways. If we choose $\theta_{n,\text{II}} = \frac{2\pi}{M}(n + \frac{1}{2}),\ n \in \{0, ... M-1\}$ (``half-index sampling, which \textit{doesn't} include the endpoints 0 and $\pi$), then we get:
...
This is the DCT-II$^{-1}$ if we take its coefficients to be the phase ramp multiplied with the DFT coefficients. Operating on samples $y_{n,\text{II}}$, taken from $\theta_{n,\text{II}}$ (``half-indices"), \textit{offsets} the phase difference and makes this equivalent to a DFT, but we once again get the benefit of only needing non-redundant information. Notice the Nyquist term, which would be at index $\frac{M}{2} = N+1$, has disappeared with this sampling, its positive and negative frequency components perfectly cancelling due to interaction with the phase ramp.
...
We can follow a similar (and now that the even-odd intuition has been covered, maybe a more direct) line of reasoning to find inverse transforms for derivatives when using the DCT-II. So far we know:\vspace{-5mm}
...
We can continue the pattern, re-discretize at $\theta_{n,\text{II}}$, and explicitly list out some terms with value zero to obtain something that looks like discrete inverse transforms for the even and odd cases:\vspace{-5mm}
...
As we take higher derivatives, we can see a bit of a pattern. In particular, the function of $x$ multiplying each derivative of $y$ in $\theta$ comes from at most two terms in the preceding derivative, which have a predictable form:\vspace{-2mm}
...
I've done the above programmatically in \href{https://github.com/pavelkomarov/spectral-derivatives/blob/main/specderiv/specderiv.py}{the code}, such that we can find and apply the factors---and thereby accomplish the variable transformation back to the Chebyshev domain---for arbitrarily high derivatives.
...
For both orders above, if we instead use type II reconstruction, then $Y_k$ become $Y_{k,\text{II}}$, and the only difference is the $k = N$ terms are multiplied by 2 like every other, instead of by 1, and can therefore be moved \textit{inside} the sums.
...
Now using this to evaluate \autoref{minexample}:\vspace{-3mm}
...
Even worse, at the \textit{edges} of the domain we still need to use L'Hôpital's rule on an analytic expression to evaluate the limits of\vspace{-2mm}
