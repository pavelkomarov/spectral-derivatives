...
Where $[:\!N+1]$ truncates to only the first $N+1$ elements. Given the equality above, we can line up everything we now know in a diagram:\vspace{-3mm}
...
\centering
\begin{tikzpicture}
	\node (y_long) at (0, 3) {$\vec{y}_{long}$};
	\node (Y_long) at (0, 0) {$\vec{Y}_{long}$};
	\node (y_short) at (4, 3) {$\vec{y}_{short}$};
	\node (Y_short) at (4, 0) {$\vec{Y}_{short}$};
